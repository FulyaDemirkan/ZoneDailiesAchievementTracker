...
ZDAT.Data.Achievements.GetStatus = function()
  ...
  for i, v in ipairs(ZDAT.Data.Achievements.DB) do
    for j, y in ipairs(v.ACHIEVEMENTS) do
      local row = {zone = "", categoryName = "", progress = "",  tierI = {id = "", name = "", description = "", icon = "", completed = ""}, tierII = {id = "", name = "", description = "", icon = "", completed = ""}, tierIII = {id = "", name = "", description = "", icon = "", completed = ""} }
      for k, z in ipairs(y.ACHIEVEMENT_IDS) do
        local name, description, points, icon, completed = GetAchievementInfo(z)
        local progress = id64(GetAchievementProgress(z))
        row.zone = v.ZONE
      ...
      --d("name: " .. row.tierIII.name .. " - progress: " .. row.progress)
    end
  end
  return status
end
